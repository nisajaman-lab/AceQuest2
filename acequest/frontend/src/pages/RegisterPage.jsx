...
import { UserPlus, Shield, Mail, Lock, User, AlertCircle, Loader2 } from 'lucide-react';

export default function RegisterPage() {
  const [username, setUsername] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const { register, isLoading, error, clearError, isAuthenticated } = useGameStore();
    ...
    <div className="min-h-[80vh] flex items-center justify-center px-6 py-12 relative">
      ...
      <div className="glass-panel-heavy p-8 md:p-10 rounded-2xl w-full max-w-md shadow-2xl relative border border-teal/20">
        ...
        <div className="absolute top-0 left-0 right-0 h-6 bg-slate-900/80 rounded-t-2xl flex items-center px-4 gap-1.5 border-b border-teal/10">
          ...
          <span className="text-[10px] font-mono text-slate-500 ml-auto uppercase tracking-wider">Secure Enrollment</span>
        </div>
        ...
        <div className="text-center mt-4 mb-8">
          ...
          <h2 className="text-2xl font-extrabold tracking-wide text-white">TSA ENROLLMENT REGISTRY</h2>
          <p className="text-xs text-slate-400 mt-1 uppercase tracking-widest font-semibold">Join Totally Spies Academy (TSA)</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="space-y-2">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider">Secure Communications Email</label>
            <div className="relative">
              <Mail className="absolute left-3.5 top-3.5 h-5 h-5 text-slate-500" />
              <input
                type="email"
                placeholder="Email"
                value={email}
        ...
        <div className="mt-8 pt-6 border-t border-slate-800/60 text-center text-xs text-slate-400">
          Already registered?{' '}
          <Link to="/login" className="text-teal-light hover:underline font-bold">
            Access Terminal
          </Link>
        </div>
      </div>
